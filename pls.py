...
# This version is going to be a cost function for pso in order to optimize regulation parameters
# Programmer: Mehrdad Kashefi


def one_neuron_cost(regulators):
    # Import libraries
    ...
    for fold_count in range(num_fold):
        ...
        """
        [prediction_train, prediction_train_no_active, prediction_test, prediction_test_no_active, a1, a2, b1, b2] = mlp_mse(feature_allband_train, force_train, feature_allband_test, force_test, regulators[0, fold_count])
    ...
    # Final Cost Function
    cost_importance = 0.5
    function_cost = 1 / (cost_importance * np.mean(fold_r2_test) + (1-cost_importance) * np.mean(fold_r_test))

    return function_cost, np.mean(fold_r2_test), np.mean(fold_r_test)
